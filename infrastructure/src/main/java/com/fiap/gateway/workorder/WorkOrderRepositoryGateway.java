...
public class WorkOrderRepositoryGateway implements WorkOrderGateway {
  ...
  public List<WorkOrderHistory> getHistoryByCustomerCpfCnpj(String cpfCnpj) {
    ...
    var orders =
        workOrderRepository.findByCustomerDocumentNumberOrderByCreatedAtAsc(cleaned);
    return workOrderHistoryMapper.fromWorkOrders(orders);
  }

  @Transactional(readOnly = true)
  @Override
  public List<WorkOrderHistory> findHistoryByWorkOrderIdOrderByCreatedAtDesc(UUID workOrderId) {
    WorkOrderEntity workOrderEntity = workOrderRepository.findById(workOrderId)
            .orElseThrow(() -> new RuntimeException("WorkOrder not found: " + workOrderId));

    List<WorkOrderHistoryEntity> entities = workOrderHistoryRepository.findByWorkOrderOrderByCreatedAtDesc(workOrderEntity);
    return workOrderHistoryMapper.toDomain(entities);
  }

  @Transactional
  @Override
  public void saveHistory(WorkOrderHistory history) {
    if (history == null || history.getWorkOrderId() == null) {
      return;
    }

    WorkOrderEntity workOrderEntity = workOrderRepository.findById(history.getWorkOrderId())
            .orElseThrow(() -> new RuntimeException("WorkOrder not found: " + history.getWorkOrderId()));

    WorkOrderHistoryEntity entity = workOrderHistoryMapper.toEntity(history, workOrderEntity);
    workOrderHistoryRepository.save(entity);
  }
}
